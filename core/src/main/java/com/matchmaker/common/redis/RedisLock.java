package com.matchmaker.common.redis;
...
public class RedisLock {
    ...
    public Boolean lock(String id, String value, int expiry, int retry) {
        ...
        try(Jedis jedis = redisService.getResourceForUrlWithPort(LOCK_REDIS_URL, LOCK_REDIS_PORT, LOCK_REDIS_PASSWORD)){
            if (jedis != null) {
    ...
    public Boolean unlock(String id) {
        ...
        try(Jedis jedis = this.redisService.getResourceForUrlWithPort(LOCK_REDIS_URL, LOCK_REDIS_PORT, LOCK_REDIS_PASSWORD)) {
            if(jedis != null) {
                String redisValue = jedis.get(key);
                if (redisValue != null) {
